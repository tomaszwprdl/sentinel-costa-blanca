...
- The latest app maintenance commit on `origin/main` is `2bc4e9ad9b844361d49f8f48b77b4b8515bf1ea9`. The Owner approved public indexing, and `2bc4e9a` removes the pre-live metadata, `robots.txt`, and Netlify header blocks. The website is intended to be publicly indexable; future indexing/robots changes still require explicit Owner approval. The website customer-journey phase is closed with a PASS VERIFIED verdict, Contact email is functional, and no active website implementation task remains. Remaining work is non-website operational readiness — see `doc/TASK.md` and "Remaining Launch Gates" in `doc/STATUS.md`.
- The final website closure consistency patch aligns the qualified individual-task exception across Services and FAQ, makes the Basic / Extended paid-action approval rule explicit in mounted Services copy, and removes the proven-unused message keys that could reintroduce an unscoped euro limit. Basic / Extended require owner approval before every paid action or vendor spend; Full keeps EUR 300 standard / EUR 500 optional. Do not reintroduce a Basic / Extended euro figure. The website is closed and in maintenance mode.
- The Owner-accepted whole-site coherence audit (Patches 1–4) is complete, committed, pushed, and deployed. It protects one semantic model, canonical PL/EN terminology, the `Rozpocznij kwalifikację` / `Start qualification` route-to-Contact CTA, and explicit page ownership. Do not restore the removed FAQ assumptions board or expand Services into a second How It Works page. The website is frozen while document production continues.
- How It Works is shipped, deployed, and smoke-verified in its current operational-proof form on `origin/main` (latest app maintenance commit `d0a9da8`; payload cleanup `b9dab063`; operational-proof app commit `eeae1dd`, deployment docs `6ca538f`, earlier smoke closure `0481852`). Its live structure is: compact hero; `Jedna obserwacja` / `One observation` case thread with route-strip visual; inspection matrix as the main physical-check visual; owner report proof; decision threshold; continuing rhythm/activation notes; and CTA. The report section centers a photographed REC-01-style example document, with compact copy above and observation/evidence/next-step cards around the image on desktop and stacked on tablet/mobile. It is example/documentary material with an explicit not-client-report disclaimer, not client proof or downloadable public paperwork. No generated damp/stain photo was added. Retired How It Works PL/EN message blocks are no longer serialized into the production payload, the EN 390px `qualification` wrap is improved, and the page canvas now uses the shared Services/About/Contact atmosphere. The active Services key `services.redesign.custody.eyebrow` (`Access chain`) was deliberately left untouched. PL/EN desktop and 390px production smoke passed with intact report-photo corners, the old native report mockup absent, zero overflow, clean console/network checks, and unchanged `noindex,nofollow`. Protected contracts are unchanged; no active website task remains.
- The page-by-page polish pass is complete. Home, Pathway states, Services, How It Works, FAQ, About, and Contact pass the current core polish gate. Do not reopen a passed page without a real regression or explicit Owner scope. Public indexing is enabled by Owner decision; the website code/design remains closed and frozen.
