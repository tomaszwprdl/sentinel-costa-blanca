...
- The Owner-accepted whole-site coherence audit (Patches 1–4) is complete as uncommitted working-tree app/content work. It protects one semantic model, canonical PL/EN terminology, the `Rozpocznij kwalifikację` / `Start qualification` route-to-Contact CTA, and explicit page ownership. Do not restore the removed FAQ assumptions board or expand Services into a second How It Works page. No active website implementation task remains; the website is frozen while document production continues.
