...
- How It Works visual-assets pass is complete locally at app commit `84112f9` (`refine: add how-it-works inspection matrix`). It adds the inspection matrix as the main physical-check visual, a compact `Jedna obserwacja` / `One observation` case-thread bridge, and a route-strip image for access → check → report → decision. Role split: case thread = example route / narrative bridge; inspection matrix = physical checks; report artifact = owner record; decision threshold = action vs owner-approval boundary. All example/schematic/report materials are example structure only, not client proof; no observation photo was added. Protected contracts are unchanged. Pending next: push and deployed smoke QA.
