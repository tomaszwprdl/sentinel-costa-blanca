...
- How It Works report proof section is pushed, deployed, and smoke-verified on `origin/main` (app commit `eeae1dd`, deployment docs commit `6ca538f`). The `Co otrzymuje właściciel` / owner report section centers a real photographed REC-01-style report document as example/documentary material, not client proof. Desktop uses a compact-top-copy evidence board with a dominant report photo and observation/evidence/next-step cards connected by restrained leader lines; tablet/mobile use a stacked layout. PL/EN desktop, PL 768px, and PL/EN 390px passed with intact photo corners, zero overflow, clean console/network checks, and unchanged `noindex,nofollow`. No generated damp/stain photo was added. Protected contracts are unchanged; no active website task remains.
- How It Works visual-assets pass is pushed, deployed, and smoke-verified on `origin/main` at `a2b6676` (app commit `84112f9`, docs closure `a2b6676`). It adds the inspection matrix as the main physical-check visual, a compact `Jedna obserwacja` / `One observation` case-thread bridge, and a route-strip image for access → check → report → decision. Role split: case thread = example route / narrative bridge; inspection matrix = physical checks; report artifact = owner record; decision threshold = action vs owner-approval boundary. All example/schematic/report materials are example structure only, not client proof; no observation photo was added. Production smoke passed for PL/EN desktop, PL 768px, and PL/EN 390px: current modules render, the old pinned journey is absent, overflow is zero, browser/network checks are clean, and `noindex,nofollow` is unchanged. Protected contracts are unchanged. No active website task remains. No housekeeping cleanup was performed; orphaned legacy How It Works code/messages remain optional future housekeeping in a separate scope.
