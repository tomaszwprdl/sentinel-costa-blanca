...
| REC-03 | Decision request template | delivered-record | draft-source | — | — | — | Y | — | Y | REC-01 | v0.2 source + DOCX candidate produced as front-decision blind-feedback micro-patch; v0.1 kept for history until Owner acceptance; next = Owner review / quick acceptance check / connected beta packet update only after Owner acceptance. | Authority basis is a referenced field; non-Full = owner approval (Option A); no invented spend limit; not customer-facing approved |
