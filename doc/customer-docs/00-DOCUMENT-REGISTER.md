...
| ID | Document | Class | Status | Owner | Law | Acct | OpTest | Path | 1stCust | Depends on | Next action | Notes |
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| SCO-01 | Scope Register (draft) | customer-facing | draft-source | Y | P | — | Y | field | Y | GOV-01, Decision Memo | SCO-01 v0.6 source + DOCX candidate produced from beta feedback (employee-led intake framing); v0.5 kept for history; next = Owner review / simulation re-fill / connected beta packet update only after Owner acceptance; legal/accountant/customer-facing approval gates remain | Dependency root for contracts/SOPs |
