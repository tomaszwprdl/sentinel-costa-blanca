...
| ID | Document | Class | Status | Owner | Law | Acct | OpTest | Path | 1stCust | Depends on | Next action | Notes |
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| SCO-01 | Scope Register (draft) | customer-facing | draft-source | Y | P | — | Y | field | Y | GOV-01, Decision Memo | Clean PL Markdown source draft created (`working/01-PROPERTY-SCOPE-CONFIRMATION-WORKING-DRAFT-PL.md`); functional DOCX working document not yet produced; next = internal review / DOCX production / simulation against private/guest/mixed cases and patch only if Owner approves | Dependency root for contracts/SOPs |
