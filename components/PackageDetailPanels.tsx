...
export default function PackageDetailPanels() {
  ...
  const [selected, setSelected] = useState<(typeof PACKAGES)[number]['key']>('green');
  const active = PACKAGES.find((pkg) => pkg.key === selected) ?? PACKAGES[0];
          ...
          <div className="grid gap-0 md:grid-cols-[minmax(0,0.55fr)_minmax(0,0.45fr)]">
              ...
              <div className="mt-6 grid gap-3">
                {active.highlights.map((key) => (
                  <div key={key} className="rounded-2xl bg-surface-light-alt p-4">
                    <p className="mb-0 text-sm font-semibold leading-relaxed text-body">{t(`${active.key}.${key}`)}</p>
                  </div>
                ))}
              </div>
            ...
            <div className="border-t border-structural-light bg-surface-light-alt p-4 md:border-l md:border-t-0 md:p-5">
              ...
              <div className="mt-4 rounded-2xl bg-surface-card p-4">
                ...
                <p className="mb-1 text-sm text-body"><strong>{t(`${active.key}.summary.accessValue`)}</strong> {t(`${active.key}.summary.accessLabel`)}</p>
                <p className="mb-0 text-sm text-body"><strong>{t(`${active.key}.summary.decisionsValue`)}</strong> {t(`${active.key}.summary.decisionsLabel`)}</p>
              </div>
            </div>
          </div>

          <div className="border-t border-structural-light p-5 md:p-8">
            <DisclosureBlock label={t('redesign.packageDetails.fullScopeLabel')} explainer={t('redesign.packageDetails.fullScopeExplainer')}>
              <div className="grid gap-5 md:grid-cols-2">
                <div>
                  <h4 className="mb-3 text-base font-black text-heading">{t('redesign.packageDetails.includedLabel')}</h4>
                ...
                <div>
                  <h4 className="mb-3 text-base font-black text-heading">{t('redesign.packageDetails.notIncludedLabel')}</h4>
                  <ul className="ml-4 list-disc space-y-2 text-sm text-body">
